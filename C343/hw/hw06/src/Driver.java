...
public class Driver {
    ...
    public void autoPlay() {
        javax.swing.Timer timer = new Timer(Constants.TIMER_WAIT, e -> this.flood(this.suggest()));
        timer.start();
    }
    ...
    public int suggest(){
        ...
        int color = 0;
        for(int i = 0; i < 6; i++){
            List<Coord> tmp = new LinkedList<>();
            for (Coord a : floodFunction.floodList){
                tmp.add(a);
            }
            for (int j = 0; j < tmp.size(); j++){
                LinkedList<Coord> sum = floodFunction.getNeighbor(tmp.get(j));
                for (Coord b : sum){
                    if (! tmp.contains(b)){
                        if (getColor(b) == i){
                            tmp.add(b);
